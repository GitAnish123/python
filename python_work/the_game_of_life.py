            ...
            if enter_pin == pin:
                ...
                while max_attempts > 0:
                    ...
                    if set(selected_word) <= set(guessed_letters):
                        ...
                        money_for_wheel_of_fortune1 = random.randint(1000, 10_000)
                        money = money + money_for_wheel_of_fortune1
                        print(f"You get ${money_for_wheel_of_fortune1}. You now have ${money}")
                        million_dollars = random.choices(['no', 'yes'], weights=[0.95, 0.05])[0]
                        wild_card_get = random.choices(['no', 'yes'], weights=[0.75, 0.25])[0]
                        if million_dollars == 'yes':
                            print(f"You have the million dollar wedge!!! You have a chance to get one million dollars.")
                            million_dollar_wedge = True
                        if wild_card_get == 'yes':
                            print(f"You have a wild card!!! You can get one extra consonent at the bonus round.")
                            wild_card = True
                        bonus_round_go = input("Do you want to enter to the bonus round: ")
                        if bonus_round_go == 'no':
                            print(f"Ok...")
                            time.sleep(2)
                            break
                        else:
                            opponent_amount = random.randint(1000, 10_000)
                            if money_for_wheel_of_fortune1 < opponent_amount:
                                ...
                                for letter in phrase:
                                    if letter == " ":
                                        ...
                                        display += "_ "
                                print("Your phrase is:", display.strip())
                                while True:
                                    starting_letters = input("Enter 'R', 'S', 'T', 'L', 'N', 'E' (without quotes) to reveal starting letters: ").strip().upper()
                                    if starting_letters == 'RSTLNE':
                                        break
                                    else:
                                        print("Please enter 'R', 'S', 'T', 'L', 'N', 'E' (without quotes).")
                                guessed_letters = list(starting_letters)
                                display = ""
                                for letter in phrase:
                                    if letter.upper() in guessed_letters:
                                        ...
                                        display += "_ "
                                print("Your updated phrase is:", display.strip())
                                while True:
                                    if wild_card == False:
                                        consonants = input("Enter three consonants separated by commas IN ALL CAPS OR ELSE IT WON'T COUNT: ").strip().split(',')
                                        if len(consonants) != 3:
                                            print("Please enter exactly three consonants.")
                                        elif any(vowel in 'AEIOU' for vowel in consonants):
                                            print("Consonants should not include vowels (A, E, I, O, U).")
                                        else:   
                                            break
                                    else:
                                        consonants = input("Enter four consonants separated by commas IN ALL CAPS OR ELSE IT WON'T COUNT: ").strip().split(',')
                                        if len(consonants) != 4:
                                            print("Please enter exactly four consonants.")
                                        elif any(vowel in 'AEIOU' for vowel in consonants):
                                            print("Consonants should not include vowels (A, E, I, O, U).")
                                        else:   
                                            break
                                while True:
                                    vowel = input("Enter one vowel IN CAPS OR ELSE IT WON'T COUNT: ").strip().upper()
                                    if len(vowel) != 1 or vowel not in 'AEIOU':
                                        print("Please enter exactly one vowel (A, E, I, O, U).")
                                    else:
                                        break
                                guessed_letters.extend(consonants)
                                guessed_letters.append(vowel)
                                print("Your letters are:", guessed_letters)
                                updated_display = ""
                                for letter in phrase:
                                    if letter.upper() in guessed_letters:
                                        ...
                                        updated_display += "_ "
                                    print("Updated puzzle:", updated_display.strip())
                                    time.sleep(1)  
                                time.sleep(2)
                                print("\nTime to solve the puzzle!")
                                start_time = time.time()
                                while True:
                                    remaining_time = max(0, 25 - (time.time() - start_time))
                                    user_guess = input(f"You have {int(remaining_time)} seconds left. Guess the phrase: ").lower()
                                    if user_guess == phrase.lower():
                                        print("Congratulations! You did it!")
                                        if million_dollar_wedge:
                                            bonus_round_reward = random.choices([40000, 50000, 75000, 1000000], 
                                                                                weights=[0.82, 0.1, 0.06, 0.02])[0]
                                        else:
                                            bonus_round_reward = random.choices([40000, 50000, 75000, 100000], 
                                                                                weights=[0.82, 0.1, 0.06, 0.02])[0]
                                        if bonus_round_reward == 100_000:
                                            print(f"YOU WIN $100,000!!! EXTRA CONGRATS!!!")
                                        elif bonus_round_reward == 1_000_000:
                                            print(f"You got the BEST REWARD.")
                                            time.sleep(0.5)
                                            print(f"You got the BEST REWARD..")
                                            time.sleep(0.5)
                                            print(f"You got the BEST REWARD...")
                                            time.sleep(1.5)
                                            print(f"YOU GOT 1 MILLION DOLLARS!!!")
                                            print(f"CONGRATULATIONS OF YOUR GREAT BENIFIT!!!")
                                        else:
                                            print(f"You got ${bonus_round_reward}!")
                                            money = money + bonus_round_reward
                                        break  
